...
- [x] `surveillance` — ✅ xong (2026-09-25): danh mục 34 tỉnh (+ dân số tham chiếu), GeoJSON theo phiên bản, quan sát theo tỉnh/khoảng tháng (giữ nhãn `data_source`), **phiên bản dữ liệu bất biến** (trigger DB chặn UPDATE/DELETE; cùng nội dung → idempotent, khác nội dung → 409), tải panel Parquet kèm `X-Content-SHA256` + cache bất biến, sự kiện `data_version.published` ghi outbox CÙNG giao dịch. Đọc Parquet bằng `parquet-go` (kiểm cột bắt buộc TRƯỚC khi đọc vì thư viện âm thầm điền 0 cho cột thiếu). Kiểm chứng: repotest trên memory + Postgres thật (gồm công bố song song, immutability ở tầng DB), test HTTP fail-closed theo hợp đồng, chạy trên container với panel v0.2.0 thật (9.326 dòng, 34 tỉnh, sha256 khớp file gốc). **`ingest-worker` hiện là lệnh CLI `surveillance import-panel`** (đi qua ĐÚNG đường kiểm của API, không đường tắt); tách thành worker riêng khi có nguồn nhập định kỳ.
