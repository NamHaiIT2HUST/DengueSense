...
**Lát cắt 2 — ✅ XONG (2026-09-25):** xem rủi ro trên **số thật** của exp_016, chạy ở chế độ demo.
- [x] Bộ component trung thực §9.2: `ProbabilityWithBaseRate` (mức nền BẮT BUỘC ở kiểu — thiếu là lỗi biên dịch, có test `@ts-expect-error`), `ProvenanceBar` (T3/T4), `ForecastFlags` (T6), `ReliabilityNote` (T5), `InputSources` (T2, hoa văn khác cho dữ liệu không thật), cùng `Segmented`, `Select`, `TimeSeriesChart`
- [x] **S2 bản đồ rủi ro** `/app/ban-do`: chọn lượt · tầm 1/2/3/6 · chỉ số tô (xác suất vượt ngưỡng / tỉ suất ca) · vùng, lưu trên URL (tham số sai bị bỏ qua, không hỏng trang); Leaflet lớp rời rạc với chú giải do máy chủ trả; tỉnh không dự báo xám; viền nét đứt cho đầu vào ước lượng; bảng xếp hạng sắp xếp được (`aria-sort`) là đường truy cập bàn phím; mọi dòng bảng có xác suất KÈM mức nền
- [x] **S3 chi tiết tỉnh** `/app/tinh/$provinceId`: 4 thẻ theo tầm; biểu đồ SVG tự vẽ (trục từ 0, phân biệt bằng kiểu nét, vùng "sau tháng neo — chỉ để đối chiếu", KHÔNG vẽ khoảng dự báo, có bảng dữ liệu thay thế); SHAP top-5 (nêu rõ chỉ là thành phần LightGBM, không phải nhân quả, đọc theo hệ số nhân e^đóng góp); độ tin cậy vùng, cờ, nguồn đầu vào
- [x] **S4 lượt dự báo** `/app/luot-du-bao`: danh sách bất biến; analyst+ tạo lượt backtest (RHF+Zod, lỗi theo trường của server, job chạy nền có tiến độ và thông báo `aria-live`); viewer thấy lý do không tạo được
- **Dữ liệu demo là số thật:** `ai-service/app/serving/forecast_api/replay.py` tái hiện mùa 2010 từ `experiments/exp_016/results.json` (m4, p_clf nguyên văn — KHÔNG fit lại) + dữ liệu dẫn xuất nhân quả từ panel (ngưỡng P75, tỉ lệ nền cửa sổ huấn luyện, nguồn đầu vào, dân số) + SHAP LightGBM fit lại như exp_014; xuất ra `public/demo/*.json` (tải theo lượt, ~165 KB/lượt, không nằm trong bundle JS). Luật trình bày (cờ, độ tin cậy, lớp màu) ở `policy.py` — service `forecast` thật dùng CHUNG bản này. Test Python đối chiếu từng con số với exp_016 và kiểm hợp đồng.
- **Sửa hợp đồng cùng lúc (cộng thêm, chưa có consumer nào bị phá):** `ForecastItem.incidence_pred_per_100k` (đầu ra gốc — mô hình dự báo tỉ suất, không phải số ca), `Explanation.explained_component` (nói rõ SHAP là của thành phần nào), định nghĩa lại `base_rate` (tỉ lệ nền của cửa sổ huấn luyện) và thang log của SHAP.
- Kiểm chứng: **144 test đơn vị/component** (cổng Đợt 1 chạy trên router thật: 2010-03 → bảng khớp exp_016, mọi dòng có mức nền; luồng đổi tầm/chỉ số/vùng/sắp xếp/tham số URL sai; chi tiết tỉnh; tạo lượt; các luật T1–T6) + **22 E2E** trên bản build, axe 0 lỗi critical/serious trên 6 trang. JS khởi đầu 149 KB / 250 KB.
- **Lỗi thật do axe phát hiện (đã sửa):** `<dl>` của dải provenance chứa liên kết.
- Chưa làm ở đợt này: khoảng dự báo (chưa có khoảng đã kiểm chứng độ phủ), nối API thật, Recharts (chưa cần — một dạng biểu đồ, tự vẽ để kiểm soát nét/nhãn theo T2/T7).
