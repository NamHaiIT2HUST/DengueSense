...
4. Test phải **100% `data_source == "real"`**, không được lẫn `estimated`/`imputed`/`simulated` ([01 §8](01-chien-luoc-du-lieu.md#8-xử-lý-dữ-liệu-không-phải-real--4-loại-không-được-lẫn-lộn)).

   ⚠️ **Hệ quả cụ thể với dữ liệu hiện tại** (đã đo thật, xem [00 §C.4](00-review-hien-trang.md#c4--quy-mô-dữ-liệu-quyết-định-lựa-chọn-model--đã-đo-thật-không-còn-ước-lượng)): cấp tỉnh chỉ có `real` cho giai đoạn **1994-2010**. Nghĩa là tập test cấp tỉnh (theo rolling-origin, [01 §7.1](01-chien-luoc-du-lieu.md#71-chia-ba-tầng-theo-thời-gian)) phải nằm trong cửa sổ này — **không thể** test trên các đợt dịch gần đây (2022, 2023) ở cấp tỉnh cho tới khi có nguồn `real` mới (NSO cấp tỉnh hoặc Đường A). Nếu tới lúc promote model mà vẫn chưa có, ghi rõ trong model card: *"đánh giá cấp tỉnh giới hạn ở giai đoạn lịch sử 1994-2010; hiệu năng giai đoạn gần đây sẽ xác nhận trong pilot"* — đây là câu trả lời trung thực chấp nhận được, không phải lỗ hổng cần giấu.
