...
| `gateway` | Cửa vào duy nhất (Go) | Chỉ chạy được khi đã đặt `GATEWAY_JWT_PUBLIC_KEY` |
...
- Dán dòng `PUBLIC=...` vào `infra/.env` (biến `GATEWAY_JWT_PUBLIC_KEY`).
...
đặt `POSTGRES_HOST_PORT`, `NATS_HOST_PORT`, `NATS_MONITOR_HOST_PORT`, `GATEWAY_HOST_PORT` trong `infra/.env`.
