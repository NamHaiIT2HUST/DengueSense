...
import { ConceptTag } from "@/shared/ui/Badge";
  ...
  return (
    <div className="glass-panel rounded-2xl p-6 sm:p-8">
      <div className="mb-6 flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1.5 max-w-lg text-sm leading-relaxed text-[var(--ink-secondary)]">
            Ví dụ minh hoạ định dạng đầu ra cho luồng B2G. Số liệu điền vào mẫu luôn lấy từ Layer
            1/2 (không để LLM tự sinh số) — xem guardrail ở docs/05.
          </p>
        </div>
        <ConceptTag />
      </div>
      ...
      <div className="rounded-xl border border-[var(--border-hairline)] bg-[var(--bg-surface-2)] p-5 font-mono text-[13px] leading-relaxed text-[var(--ink-secondary)]">
        ...
        <p className="text-[var(--ink-primary)]">Kính gửi: Trung tâm Kiểm soát bệnh tật {name}</p>
        <p className="mt-3">
          Căn cứ dữ liệu giám sát dịch tễ và kết quả mô hình dự báo, khu vực{" "}
          <b className="text-[var(--ink-primary)]">{name}</b> hiện có điểm rủi ro{" "}
          <b className="text-[var(--ink-primary)]">{score}/100</b>, thuộc nhóm cần theo dõi ưu tiên
          trong 30 ngày tới.
        </p>
        <p className="mt-3">
          Đề nghị: (1) tăng cường giám sát ổ dịch tại khu vực dân cư mật độ cao; (2) chuẩn bị vật tư
          phun hoá chất theo phương án phân bổ đính kèm; (3) đẩy mạnh truyền thông diệt lăng quăng
          tới hộ dân.
        </p>
        <p className="mt-3 text-[var(--ink-muted)]">
          [CẦN CÁN BỘ BỔ SUNG]: đơn vị đầu mối tiếp nhận vật tư, thời hạn báo cáo kết quả triển
          khai.
        </p>
      </div>
      ...
      <div className="mt-5 flex flex-wrap items-center justify-between gap-3">
        <span className="text-[12px] text-[var(--ink-muted)]">
          Cơ chế human-in-the-loop — không có đường nào ban hành mà không qua duyệt
        </span>
          ...
          {decision ? (
            ...
            <div className="flex gap-2">
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-full bg-[var(--accent-solid)] px-4 py-2 text-[13px] font-semibold text-white transition-transform hover:scale-[1.03] active:scale-[0.98]"
              >
                <Check size={14} /> Duyệt
              </button>
