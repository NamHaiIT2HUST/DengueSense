  ...
  return (
    <section id="overview" className="relative overflow-hidden px-6 pb-20 pt-16 sm:pt-24">
      ...
      <div className="relative mx-auto max-w-5xl text-center">
        ...
        >
          DengueSense tích hợp AI dự báo, tối ưu hoá phân bổ nguồn lực và GenAI soạn lệnh điều phối
          — chuyển phòng chống sốt xuất huyết từ bị động sang chủ động.
        </motion.p>
        ...
        >
          <a
            href="#map"
            className="inline-flex items-center gap-2 rounded-full bg-[var(--accent-solid)] px-6 py-3 text-sm font-semibold text-white transition-transform hover:scale-[1.03] active:scale-[0.98]"
          >
        ...
        >
          {PILLARS.map((p) => (
            <div key={p.title} className="glass-panel rounded-2xl p-5 text-left">
              <p.icon size={20} strokeWidth={2} className="mb-3 text-[var(--accent)]" />
              <h3 className="font-display text-[15px] font-semibold">{p.title}</h3>
              <p className="mt-1 text-[13px] leading-snug text-[var(--ink-muted)]">{p.desc}</p>
            </div>
          ))}
        </motion.div>
      </div>
