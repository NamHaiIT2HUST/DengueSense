...
export function OptimizationPreview({ provinces }: { provinces: ProvinceRisk[] }) {
  ...
  const ranked = useMemo(() => {
    ...
    const sorted = [...withCost].sort((a, b) => b.risk_score / b.cost - a.risk_score / a.cost);
  ...
  return (
    <div className="glass-panel rounded-2xl p-6 sm:p-8">
      <div className="mb-6 flex flex-wrap items-start justify-between gap-3">
        <div>
          <h3 className="font-display text-xl font-semibold">Layer 2 — Tối ưu phân bổ nguồn lực</h3>
          <p className="mt-1.5 max-w-lg text-sm leading-relaxed text-[var(--ink-secondary)]">
            Kéo thanh ngân sách để xem thuật toán chọn tỉnh nào được ưu tiên. Bản demo dùng greedy
            theo tỉ lệ rủi ro/chi phí — bản thật (MILP, OR-Tools) tối ưu <b>số ca giảm được</b>,
            không chỉ xếp hạng rủi ro.
          </p>
        </div>
        <ConceptTag />
      </div>
      ...
      <div className="mb-7 rounded-xl bg-[var(--bg-surface-2)] p-5">
        ...
        <input
          type="range"
          aria-label="Ngân sách khả dụng"
          aria-valuetext={`${new Intl.NumberFormat("en-US").format(budget)} đô la`}
          min={50_000}
          max={1_500_000}
          step={25_000}
          value={budget}
        ...
        <div className="mt-4 flex gap-6 text-[13px] text-[var(--ink-muted)]">
          <span>
            <b className="text-[var(--ink-primary)]">{selectedCount}</b>/34 tỉnh được chọn
          </span>
          <span>
            Đã dùng{" "}
      ...
      <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-4 lg:grid-cols-6">
        {ranked.map((r) => (
          <motion.div
            key={r.province_id}
            animate={{ scale: r.selected ? 1 : 0.96 }}
          ...
          >
            <div
              className={`truncate text-[11px] font-medium ${
                r.selected ? "text-[var(--ink-primary)]" : "text-[var(--ink-muted)]"
              }`}
            >
              <span className="sr-only">{r.selected ? "Được chọn: " : "Không được chọn: "}</span>
              {r.name}
            </div>
